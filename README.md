...
# 概要
## どの期間で体重を何キロにしたいのか目標の設定をすることで1日の摂取カロリーが算出される。その摂取カロリーの食事を設定した期間まで続ければ栄養学の観点から目標の体重に到達することを目的としたアプリケーションである。本アプリでは、食事制限のみでダイエットしようというものではないため、あくまでそれぞれの目標に合った食事をサポートするものです。そのため、日々のトレーニングを自身でしてもらう必要があります。その結果、リバウンドのしにくく、健康で理想的な体を手に入れることができます。
